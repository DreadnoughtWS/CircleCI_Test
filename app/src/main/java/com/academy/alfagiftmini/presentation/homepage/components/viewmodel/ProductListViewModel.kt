...
class ProductListViewModel @Inject constructor(private val useCase: ProductListDomainUseCase) :
  ...
  suspend fun getProductGratisProductOrder(
    ...
    return useCase.getProductGratisProductOrder(
      type = type,
      order = order,
      sort = sort,
  ...
  suspend fun getProductSearchProductOrder(
    name: String,
    order: String,
    sort: String,
    type: String
  ...
  suspend fun getProductSearchProduct(
    name: String,
    type: String
  ): Flow<PagingData<ProductListPromotionProductDomainModel>> {
  ...
  suspend fun getBannerProduct(
    bannerId: Int,
    order: String,
    sort: String,
    type: String
  ): Flow<PagingData<ProductListPromotionProductDomainModel>> {
    return useCase.getBannerProduct(
      scope = viewModelScope,
      bannerId = bannerId,
      order = order,
      sort = sort,
